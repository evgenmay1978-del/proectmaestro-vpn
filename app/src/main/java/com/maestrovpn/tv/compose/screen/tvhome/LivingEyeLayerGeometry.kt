...
internal fun livingEyeLashes(
    ...
    fun lashes(specs: List<LivingEyeLashSpec>, upper: Boolean): List<LivingEyeLash> {
        ...
        return specs.map { spec ->
            ...
            // The shaft first projects over the moist ocular edge before curling out.
            // Painting its entire length onto the dark green surround erased its silhouette.
            val rootProjection = length * if (upper) 0.72f - 0.32f * phase else -0.45f
            LivingEyeLash(
                root = root,
                control1 = LivingEyeLayerPoint(x + dx * 0.38f - curl * 0.25f, root.y + rootProjection),
                control2 = LivingEyeLayerPoint(x + dx * 0.78f + curl, root.y + dy * 0.88f),
                tip = LivingEyeLayerPoint(x + dx + curl * 0.20f, root.y + dy * 0.68f),
                width = layerFit.mapSourceLength(spec.width),
                alpha = (spec.alpha + 0.22f).coerceAtMost(1f) * if (upper) 1f else 1f - 0.7f * phase,
                upper = upper,
            )
        }
    }
