...
Дополнение 17:10 UTC: x-ui runtime guard установлен на S1/S3/S4, первый штатный
запуск consistent, VPN-процессы сохранены. Тестовый аккаунт владелец удалит сам.
STRAVO 1.0.2 получил замечание владельца; исправления 1.0.3 — в отдельном repo `stravo-vpn`.
...
Каноническая ветка `codex/yandex-cdn-whitelist-task3-sync`, текущий локальный HEAD `9ba9c4d` (remote пока `5c288d2`). Серверы owner-confirmed working; серверное расследование завершено. Backend renewal bridge — `16aed67`; phone CDN fixes — `656430a` и локальный `9ba9c4d`. Отправленный APK собран до `9ba9c4d`, поэтому CDN labels/servers в нём ещё могут пропадать. ТВ без CDN, main/stable OTA не менять, покупки/подписки/балансы/owner binding `wapmix` сохранять.

Коммит `16aed67` добавляет `customer_renewal_whitelist.go`, адресный SQLite regression и вызов из `RenewCustomer`; соседние customer tests проходят. Phone regression/APK checks прошли в `34941407491`; preview PNG сохранены, но физический Samsung S26 Ultra не подключён. Последний `9ba9c4d` ещё не запушен и не попал в APK. Production install/restart, серверы, TV и OTA не выполнять. Не повторять полный `/db/load`, `354a727`, новые покупки или длинный preview без новой наблюдаемой ошибки.
