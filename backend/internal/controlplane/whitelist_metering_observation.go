...
const (
	...
	// pass reaches the renewal again. A four-minute margin keeps the renewal
	// ahead of any single slow reconciliation pass.
	whiteListReceiptRenewalMargin = 4 * time.Minute
)
...
func (s *Service) whiteListMeteringReadinessFromState(ctx context.Context, entitlementID, exitID string, allowAwaiting bool, requiredDesired map[string]WhiteListSidecarDesired, state whiteListSidecarRuntimeState, shared ...*whiteListPublicationOriginSnapshot) (int64, int64, bool) {
	...
	for _, origin := range origins {
		...
		if allowAwaiting && (((mode == "bytes" || observed == 0) && whiteListContainsUser(origin.unavailable, email)) ||
			(requiredDesired == nil && !whiteListContainsUser(origin.desired.ManagedUsers, email))) {
			awaiting = true
			continue
		}
...
func (s *Service) refreshWhiteListMeteringReadiness(ctx context.Context, workerID string, state whiteListSidecarRuntimeState, resolve func(string) (ExternalActionSender, bool)) (bool, error) {
	...
	for index := range results {
		...
		renew = renew || !receipt.ExpiresAt.After(s.clock.Now().Add(whiteListReceiptRenewalMargin))
	}
