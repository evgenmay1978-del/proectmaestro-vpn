...
m.bot=Bot()
# Match the pre-existing live schema without reading any customer data.
with m.get_db() as db:
 if "sub_url" not in [r[1] for r in db.execute("PRAGMA table_info(subscriptions)")]:
...
async def case(name,fn):
 ...
 except Exception as e:results.append({"case":name,"ok":False,"error":type(e).__name__})
async def run():
 st=State()
 async def add():
 ...
 async def ordinary_pay():
  class FailBot(Bot):
   async def send_message(self,*args,**kwargs):raise RuntimeError("fixture delivery outage")
  m.bot=FailBot();await m.cb_paid(CB("paid:30:400",202))
  assert m.db_pending_count()==1
  m.bot=Bot();await m.cb_paid(CB("paid:30:400",202))
  assert m.db_pending_count()==1
  with m.get_db() as q: row=dict(q.execute("select * from payments").fetchone())
  assert any(uid==101 and kw.get("reply_markup") for uid,_,kw in sent)
  await m.cb_approve(CB("approve_id:"+str(row["id"])))
  expiry=users["qa_user"]["expiresAt"]
  await m.cb_approve(CB("approve_id:"+str(row["id"])))
  assert users["qa_user"]["expiresAt"]==expiry and m.db_pending_count()==0
  await m.cb_paid(CB("paid:60:800",202))
  with m.get_db() as q: row=dict(q.execute("select * from payments where status='pending'").fetchone())
  await m.cb_reject(CB("reject_id:"+str(row["id"])))
  assert users["qa_user"]["expiresAt"]==expiry and m.db_pending_count()==0
 await case("ordinary payment retry and approval",ordinary_pay)
 async def cache():
  m.db_set_sub(202,"qa_user",datetime.now()+timedelta(days=20))
  m.db_set_sub_url("qa_user","https://subscription.invalid/sub/fixture")
  m.db_set_sub(202,"qa_user",datetime.now()+timedelta(days=25))
  assert m.db_get_sub_url("qa_user")
  m.db_set_sub(202,"other_fixture",datetime.now()+timedelta(days=25))
  assert m.db_get_sub_url("other_fixture") is None
  m.db_set_sub(202,"qa_user",datetime.now()+timedelta(days=25))
 await case("subscription link cache",cache)
 async def delete():
