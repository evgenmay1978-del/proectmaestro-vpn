...
## 0A. LIVE: ???????? component kit ?? 31 PNG ?????

???? checkpoint ????? ???????? ???????? ??????????? ??????? ? ?????????? ??? Claude/Codex ????? ?????????? ????????? ???????.

- ?????: `codex/mobile-4d-interface`.
- Asset-only commit: `af36016` (`design: add mobile 4D component kit`).
- ???????????? ?????????: `a639fa4` (design spec) ? `176ea73` (implementation plan).
- ??????? ?????: `design/mobile-asset-redraw/kit/`.
- ?????? ?????? ? 31 ??????????????? PNG:
  - 7 ????????? ?????? ???? ? ?? ???????????? ????????;
  - 3 ??????????? ?????? ?????? ???????;
  - 15 ????????? ???????????? ???????????? ?????? ? ?????????;
  - 3 ?????? `connected` / `connecting` / `error`;
  - 3 ?????????? ?????? `telegram` / `whatsapp` / `max`.
- ??????: RGBA PNG, 8 ???, sRGB ??? ICC; `home_arc_*` ? `home_console_*` ????? ?????? 2160 px; ?????????? ?????? ? 192?192.
- ?????????? ???? ??????????? ?????? ??? ?????????? ????????: ??? ??????? ?? ?????????? ? ?? ?????????????.
- ???????: `FULL PASS 31/31`; ?????? inventory, ?????????? ????, ?????????? chroma-key, ?????????? alpha ????????? ????????? ? ????????? ???????.
- Aggregate kit SHA-256: `237995f659fa4b94637d4091e7dc417ed47babb57f7836d8608fa88f32a12a1b`; ????? ?????? 8 085 225 ????.
- Scope gate: diff `app/`, `ops/`, `.github/`, TV ? `tvm_*` ????; Gradle/APK, merge, release ? OTA ?? ???????????.

????????? ?????????? ??? ??? Claude:

1. ?????? `docs/superpowers/specs/2026-08-01-mobile-4d-component-kit-design.md` ? `docs/superpowers/plans/2026-08-01-mobile-4d-component-kit.md`.
2. ????? ????????? ?????? ?? `design/mobile-asset-redraw/kit/`; ?? ?????? ??????? ??????? ? ?? ??????????? ????? UI ?????? ??????? mobile-only ??????????.
3. ??????????? ? runtime ????????? ????????? ??????????? ??????? ? ????????? ??????????? mobile-only UI ? ??????? TV-diff.
