## Текущие замечания: глаз и список CDN — 10.09.2026

Сохранять утверждённый интерфейс дерево/золото/изумруд. Никаких4D, параллакса, покачивания или пульсации света. Разрешены естественные движения глаза и настоящее смыкание зелёных век; рамка и орнамент неподвижны. Два новых отдельных образца закрытого глаза отклонены; результат показывать целым экраном и в движении. Возврат взгляда/зрачка и чистая склера подготовлены в исходниках, OFF и native preview ещё не завершены. По просьбе владельца удалены13 ненужных старых референсов; утверждённые макеты и ресурсы приложения сохранены.

Владелец подтвердил пустой CDN-список на мобильной сети. Сейчас controller34ba411 установлен: native expiry отделён от stable срока, время перепроверяется после чтений без продления сроков. Баланс PUBLISHABLE, прямой runtime200/4профиля, но3268ms/wireTTL1s несовместимы с Android1500ms и его freshness. Продолжать устранение повторных чтений без обхода допуска; не повторять старое lease-расследование. Обычная панель, front/env, боты и суммы сохранены. Текущий APK1016001 не содержит нового глаза; следующая версия должна обновлять существующую с той же подписью/package и сохранять ТВ. Сборки/эмулятор/адресные тесты только GitHub.
