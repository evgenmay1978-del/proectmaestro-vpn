## Текущая граница: только исследование CDN и VLESS — 10.09.2026

Прямое указание владельца: полностью остановить работу над существующим приложением, глазом и мобильным интерфейсом. Кандидат 1016002 не выпускать, не устанавливать и не включать в OTA. Удаление текущего кода не поручено; сохранить подготовленную работу. Новое приложение с нуля на существующей инфраструктуре и заново спроектированный интерфейс — возможный последующий этап, а не текущая задача.

Сейчас разрешено только исследование стабильности CDN и, отдельно, нестабильности обычного VLESS. Различать обнаружение трафика провайдером/ISP/DPI и распознавание выходного IP сервисами Google/OpenAI. Искать актуальные проверяемые методы максимальной приватности и устойчивости, отделять подтверждённое источниками от предположений. Не менять серверы, приложения, ботов, платежи и подписки. До получения источников не записывать исследовательские выводы как установленные факты.

Владелец подтвердил, что текущий маршрут через Yandex обходит ограничение мобильной сети; любые рекомендации по Xray принимать только после проверки первичными источниками и не считать их разрешением на изменение.

Последняя сохранённая точка: controller b14738d установлен; прежнее наблюдение публичного runtime — HTTP 200, 4 профиля, 1790 ms, wireTTL 4 s. Это прошлый замер, не новая проверка. APK-кандидат 1016002 доступен отдельным объектом; OTA_changed=false, текущая OTA остаётся 160/1016001. Реальное CDN-подключение нового APK владельцем не подтверждено.

Контекст обязателен всегда: после каждого существенного результата, изменения разрешения или блокера в этой же сессии актуализировать AGENTS.md и CONTEXT_HANDOFF.md в обоих рабочих каталогах. Фиксировать наблюдаемое состояние, незавершённый шаг и откат. Исторические разрешения на разработку и выпуск ниже не отменяют текущую остановку.
