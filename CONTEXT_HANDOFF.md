...
- Replacement S1 подтверждён как Litnets `srv_142257` / `193.17.183.48`.
  ...
  TCP/22 доступен, но SSH password-аутентификация отвергает и `root`, и
  штатного пользователя `ubuntu`. Повторная переустановка запрещена: она уже
  выполнялась и не устранила отказ доступа.
- Штатная Web-консоль показывает загруженную Ubuntu с приглашением `login:`,
  однако новые соединения к консольному прокси Litnets получают HTTP `502 Bad
  Gateway`. Официальная справка требует физическую клавиатуру и не поддерживает
  вставку из буфера обмена в консоль.
...
- В официальный Litnets ticket `#4139` отправлен подтверждённый ответ без
  секретов: запрос сбросить root-пароль внутри Ubuntu либо установить публичный
  MaestroVPN SSH-ключ. До доступа не запускать пустой panel или DNS cutover.
- Уточнение по ticket `#4139`: первое сообщение с длинным публичным ключом
  плохо отображается на мобильном экране и не является действующим запросом.
  Отправлено и проверено короткое исправление: не переустанавливать VPS,
  сбросить root-пароль внутри Ubuntu, обновить его в личном кабинете и
  проверить SSH-вход. Публичный ключ из первого сообщения пока не
  использовать.
