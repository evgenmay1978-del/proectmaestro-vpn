...
09.09.2026 после установки 88f567c владелец запросил точную диагностику рабочего теста и коммерческого CDN и спросил об обновлении Xray. Выполнены только адресные чтения конфигураций, версий, существующих журналов и отчётов. Обнаружен подтверждённый блокер контроллера: final receipt drain отклоняет более ранний отчёт другого exit на том же origin; допуск коммерческих клиентов не возобновляется. Подробности и предложенная минимальная правка в CONTEXT_HANDOFF.md. Владелец разрешил «применяй»: подготовленная правка origin+exit внесена в source; разрешены сборка контроллера на GitHub и установка только контроллера CDN. Новые тесты и тестовый CI не запускать. Результат установки ещё не наблюдался. Не выдавать найденный блокер за подтверждение исправленного подключения.
