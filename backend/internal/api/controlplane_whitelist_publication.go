...
func (b *ServiceBusiness) applyWhiteListPublication(ctx context.Context, token string, options subscriptionRenderOptions, ordinary SubscriptionSnapshot) (SubscriptionSnapshot, error) {
 ...
 now := b.requestNow(); timed, cancel := context.WithTimeout(ctx, b.cfg.WhiteListPublicationTimeout); defer cancel()
 publication, err := b.cfg.WhiteListPublicationSource.WhiteListPublication(timed, token, now)
 if err != nil || publication.Verdict != WhiteListPublishable || publication.ProjectionVersion <= 0 || publication.DesiredGeneration <= 0 || publication.FreshThrough.IsZero() || !publication.FreshThrough.After(now) || len(publication.Nodes) == 0 { return ordinary, nil }
 ...
 sum := sha256.Sum256(ordinary.Document); ordinary.ETag = fmt.Sprintf("\"%x\"", sum); ordinary.ContentLength = len(ordinary.Document)
 return ordinary, nil
}
