# Актуальные требования владельца — 06.09.2026

Зафиксировано по прямому запросу: «Запиши все. Мои требования дабы не забыл». Этот раздел имеет приоритет над прежними процессными требованиями ниже. Историческое задание сохранено целиком.

1. Нужен работающий связанный сервис: серверы, панель, Telegram-боты, MaestroVPN и подписки внешних клиентов. Код, CI и документация сами по себе не означают готовность.
2. Новые тесты, тестовые прогоны и автоматический тестовый CI — только по прямому указанию владельца. Необходимая сборка, настройка и наблюдение результата разрешены. Не затягивать работу повторными исследованиями, аудитами и документацией.
3. MaestroVPN активируется логином. В INCY/Happ и других клиентах импортируется подписка с обычными узлами и доступными CDN-узлами, понятными названиями и флагами стран.
4. Обычная подписка остаётся безлимитной по трафику; учёт и расход гигабайтов относятся только к CDN (последнее уточнение владельца06.09). CDN продаётся отдельно от обычной подписки — пакетами гигабайтов через Telegram. Клиент должен видеть назначение CDN, размер и цену пакета, понятный путь оплаты, остаток и состояние покупки. Не создавать фиктивные оплаты или повторные начисления.
5. Если Telegram недоступен из-за белых списков, покупка не должна зависеть только от него. Реализуется кабинет на доступном CDN HTTPS-адресе с существующими заказами и подтверждением оплаты владельцем.
6. CDN используется для белых списков мобильного оператора. При подключении Wi-Fi CDN автоматически останавливается и включается обычное подключение из подписки. Купленные CDN-гигабайты на Wi-Fi расходоваться не должны. Возврат в мобильную сеть не включает платный CDN без нового выбора пользователя; обычный авто-режим не должен незаметно выбирать CDN.
7. В MaestroVPN правило Wi-Fi действует в сервисе и движке, а не только в тексте/кнопке. Для внешнего клиента нужно подтвердить наличие его собственной автоматики; сервер и универсальная ссылка не доказывают тип сети телефона. Не обещать неподдерживаемый автоматический запрет в INCY/Happ.
8. В панели можно включить/выключить CDN для логина и добавлять гигабайты от1ГБ до требуемого владельцу неограниченного администрирования. Сохранять существующие реальные балансы и обычный доступ.
9. Изучить публичную реализацию покупки/гигабайтов у «Аконита», перенять понятный пользовательский путь. Маленькую цену конкурента автоматически не копировать: отдельные пакеты должны оставлять владельцу хорошую прибыль с учётом CDN, серверов, комиссий и налогов. Утверждены и установлены цены CDN:1ГБ29₽,5ГБ99₽,10ГБ199₽,25ГБ449₽,50ГБ799₽. Старые заказы сохраняют прежние суммы. CDN-безлимит не согласован.
10. Сначала закончить подключение сервиса, затем собирать APK (уточнение владельца06.09). Дать доступный для скачивания APK; сохранять возможность обновления поверх предыдущей версии, если доступен её ключ подписи. Ничего не публиковать в OTA без отдельного прямого указания.
11. Требование к глазу сохраняется отдельно: менять только внутренность медальона; исходная текстура5393.jpg — изумрудная зернистость, крупный нерегулярный рисунок, тонкие золотистые прожилки. Реалистичные объёмные верхнее/нижнее веки, складка, уголки, влажная линия, глазное яблоко позади век, зелёная радужка, круглый зрачок, отражения и мягкие тени; без ресниц/бровей. Сохранить рамку, геометрию компонента, остальной интерфейс, анимации и VPN-логику. Показать до/после.
12. Использовать ранее данные разрешения и доступы; сохранять данные, секреты, рабочие VPN/SSH и приватную рабочую whitelist-подписку. Не трогать/не зондировать приватный canary, не использовать AdminVPS. Общаться кратко по реальным изменениям и настоящим блокерам.
