...
Add one deliberately pre-candidate canary package, one dedicated first-canary runtime staging controller and one purpose-built canary command. `release.NewCandidate` is not used to prepare the first live test because it correctly requires signed results for `xray_config_test`, `direct_origin`, `yandex_get_body` and the remaining release gates; requiring that object before running those gates would be circular. After the canary produces the live evidence, the normal immutable release path may consume it.
