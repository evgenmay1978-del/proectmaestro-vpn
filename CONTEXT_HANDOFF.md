...
- Последний проверенный Android/native source: `458fec7c885250f65d474204061dcdd60d71ee56`; GREEN backend/runtime caller — `ab1a47ba11e172691454eead6ba5cbb27c772c55`, managed commercial bundle — `2a7d61c198c07bc033a09bcb8a4b6e3728bb2556`. Их exact CI ниже SUCCESS; bundle staged и real-config plan на S4 PASS, APPLY не выполнялся. Владелец установил test APK, активировал логином и сообщил ordinary VPN PASS; CDN скрыт при прежнем live backend, новый panel не deployed.
...
- Customer preparation producer завершён в4a922a0e13e4e25e38cd5d20e937f8f9af37733e (11Go files), отдельный tooling workflow вf1d1c407df5c69a8bd5d31e6ba53c4a783e9b25e. На этом exact SHA push HA [33992254105](https://github.com/evgenmay1978-del/proectmaestro-vpn/actions/runs/33992254105), job101376447259, SUCCESS: Go/race/vet/actual rqlite integration/immutable panel. Panel artifact9977110359, zip11625420bytes, SHA256 `b9447d41b3bd6a0efa1caac4ef771c430f7dbe76bcadc54d0a98e3978206d2f0`. Новый panel не installed; run terminal, не повторять.
- `maestro-import capture-xui` читает protected raw customers и существующие XUI identities через isolated same-origin verified HTTPS GET-only client с полным запретом redirects; обычный XUI New не менялся. Config допускает1–3 фактически нужных S1/S3/S4 nodes; все source/server bindings проверяются до GET. `normalize` сохраняет login/token/UUID/original Naive username/devices/precise expiry/per-node SubID, strict JSON rejects duplicates/typed aliases/lossy Unicode. Raw source SHA проверяется до/после capture; explicit max-capture-age, inventory sources orders/trials/settings/principals и protocol-server map обязательны. Full logical revision1; delta только от authenticated initial full, unchanged сохраняетrevision, changed +1, conflict/removal/overflow fail-closed. Output0600 atomic no-replace после реального Plan/protection validation.
- Этот output имеет явный scope `customer-preparation-v1`: прочие sources помечаются absent либо present-unconverted с hash. CLI `--mode apply` отвергает preparation до report/runtime factory/network; dry-run разрешён. Это НЕ полный raw orders/trials/settings/principals importer и НЕ customer cutover. Existing active logins/production sources этим checkpoint не изменялись.
- Exact tooling push run [33992254121](https://github.com/evgenmay1978-del/proectmaestro-vpn/actions/runs/33992254121), job101376447530, SUCCESS: Linux amd64 CLI и source-bound two-member artifact9976981682, zip2880752bytes, SHA256 `195f3b26d6da8201289e49dadcb4edaeaa070494f4c6e0e4618a526d185ff564`. Binary6938772bytes SHA256 `dc6a126edfaf3804377489f317e68584d255245893a23835e3d7fdbba4333e73`; manifestSHA `3d5276bfa4dd215b79e3f9dc71bfa57bc7ec4f73cc495c337a26939461387a78`. После exact CI/review CLI+manifest staged в новом protected legacy-import-tool-f1d1c40 каталоге существующего current S1 controller backup; executable не запускался, customer data не читались, services не менялись. Protected projectless receipt: work/legacy-import-tool-staged-f1d1c40.json.
- Runtime caller и final-tail integration реализованы в13cda9ed3ec992bf0c8e6e9825a853b78c05f54f; WG import-cycle исправлен вea60a30ec54aab32429dddd5afe0424a9609fa70, наблюдаемые fixture contracts — вab1a47ba11e172691454eead6ba5cbb27c772c55. Текущая подтверждённая точка06.09 описана ниже; прежний WIP allowlist больше не действует.
...
### Текущий runtime/account checkpoint06.09

- Активация MaestroVPN остаётся только по логину. Повторная установка APK/manual subscription import не требуются. Новые работы относятся к серверному CDN того же аккаунта; ordinary VPN уже подтверждён владельцем, live CDN пока не включён.
- Read-only current S1 source-shape:49customers; VLESS/VLESS3/VLESS4=48, Hy2/Naive/AnyTLS=49, WG=27. Единственный no-VLESS имеет Hy2+Naive+AnyTLS, без secondary VLESS/WG. Producer сохраняет полный per-account WG tuple через typed awg-identity-v1 AAD; renderer не наследует общий WG и сохраняет engine-version gate. Валидный no-VLESS не создаёт UUID/SubID/HMAC(empty), capture не делает XUI lookup. WG value принадлежит controlplane, subgen.WGCreds — точный alias; обратный import subgen в controlplane создаёт cycle. Reports: projectless work/legacy-capture-readiness-20260906.json и legacy-capture-shape-20260906.json. Прочитанные credentials не выводились.
- Read-only TLS prerequisite: configured S1 XUI HTTPS прошёл проверку сертификата; у S3/S4 verified HTTPS не configured. Authenticated XUI capture ещё не выполнялся. Не отправлять Bearer по публичному plaintext HTTP; нужен защищённый способ получить authoritative identities. Старый staged CLI f1d1c40 не поддерживает новые WG/no-VLESS и не должен запускаться для этого source.
- Local caller использует существующий mTLS Commander, schema2/BOOTTIME с проверкой exact physical boot/config/domain до/после RPC. Commercial mode явно defaultOFF; true допускается только для отдельного S4 API28082 и commercial PID. Usage фиксирует nonce/read-start до чтения counters, максимальный deadline=start+5s. Durable whole command записывается до RPC; unknown retry сохраняет tuple/generation/deadline. Apply/Refresh/Recover не выдаёт grant. Fence сохраняет настоящий final receipt до RemoveUser/AddUser, затем новый nonce после debit+ACK. Final evidence доступен независимо от current receipt; ACK привязан к ID+proof hash, удаляет только подтверждённую запись.
- Backend использует общий с public delivery настоящий all-Origin publication evaluator после observations/debits. Баланс сам по себе не разрешает lease. All historical-Origin final pages дренируются до новых grants; pending request восстанавливается verbatim из agent journal. Actual final proof связывается с historical desired/receipt/admission/period; только настоящий fenced_unused без counters допускает missing successful bootstrap receipt. Его ACK не создаёт metering event. Real fenced даже0/0 требует полный successful receipt и paid-period binding.
- Final proof/event/accepted sequence/outbox принимаются атомарно, затем actual debit и ACK. Unknown commit повторяет exact EventID/sequence/arithmetic mode; доказанный rollback не резервирует sequence отдельно. Для terminal tail разрешён более ранний actual timestamp только после другого Origin в том же period. Same-Origin order/reset и current-period CAS не ослаблены. Переход периода и потерянный terminal tail при physical crash остаются live gates; нельзя подставлять время или обнулять counters.
- Exact backend HA [33995536542](https://github.com/evgenmay1978-del/proectmaestro-vpn/actions/runs/33995536542) наab1a47ba11e172691454eead6ba5cbb27c772c55, job101385325153, ALL SUCCESS: Go/race/vet, actual rqlite integration, ordinary full/delta parity и immutable panel. Artifact9978075026, zip11727820bytes, SHA2568b5cc64ab3abe25ca4dc90b3ac04f10bc528421bbafc1b86ca5c2d1d5cad9c72. Panel не installed. HA33994871527/tooling33994871482/release-template33994877519 на9988b9e остановились на одном WG import cycle; HA33995148442 на75fc314 — на четырёх исправленных fixtures. Все перечисленные runs terminal; не повторять. Нельзя DELETE immutable receipt в fixture: unknown Apply моделируется потерей ответа; balance projection mutation требует version+1.
- Tooling [33995148444](https://github.com/evgenmay1978-del/proectmaestro-vpn/actions/runs/33995148444) на75fc314e996a40a24378551e27c6667026eb3a7f SUCCESS; artifact9977822588, zip2884982bytes, SHA256 f827a1c7ac4b9214b67bd5ce2ac8fc14897411cf81a2c05e5f22a91d74807015. Это customer-preparation CLI, не полный importer; на S1 новая версия не staged/invoked. Producer/subgen dependency changes теперь автоматически запускают tooling workflow.
- Managed commercial bundle2a7d61c198c07bc033a09bcb8a4b6e3728bb2556: exact [33995266529](https://github.com/evgenmay1978-del/proectmaestro-vpn/actions/runs/33995266529) ALL SUCCESS — policy, Go/race/vet, release-template, reproducible runtime+agent из одного source, реальный runtime с production template. Artifact9977869244, zip51029451bytes, SHA256 ff49cdde0feb44ef7e24e8180e3e656bbceb2bfd59209ee6bb7e9f54ae04d4d4. Manifestv2 описывает managed source/pinned Xray/schema2/BOOTTIME, только S4; upstream archive hash ему не приписывается. Existing units/legacy bundle сохранены, release-v3/status/rollback различают runtime variants. Focused local29Python tests PASS, один actual-binary test выполнен в CI.
- Этот bundle verified и staged на S4 в существующем protected backup/candidate-2a7d61c; shipped operator plan с настоящими commercial inputs/certificates PASS. Proposed release2a7d61c198c0-s4commercial-2a7211e4ddb1ecd9; runtime SHA52333619ae09a0ae2377bfd6bfa1bf86499f0ab7481707950838dad70652caf6, agent SHA50ddfbf10ad521a3841b8d44bbfc94fa75a4c07f6b089f8d41122e0315892e3d, manifest SHA18dd5ceb90da2520d81feeb4fa338074d0b051dfb9362f63e2c70ff1a5462a7f. Protected receipt: projectless work/managed-commercial-staged-2a7d61c.json. APPLY НЕ выполнялся, current/services/firewall/private canary не менялись. Нужны применимые old-runtime final-tail/backup/rollback/live gates до switch; одна plan/CI не является installed readiness.
- Следующий importer scope подтверждён source-read: orders raw содержит id/tariff/days/rub/code/login/status/sub_token/created_at/credited; исторические тарифы и buyer mapping нельзя выдумывать. Trials содержит обе redeemed_anchors/redeemed_drm HMAC maps и audit, исходных anchors нет: сохранить used навсегда и настоящий legacy salt; текущий mandatory CurrentHMAC требует честного legacy-only lookup. Native generic settings/principals файлов нет: нужны точный источник env/OLC/WDTT config/password-file и owner principal mapping. Existing settingStatements/principalStatements записывают LegacyEncryptedSecret напрямую, а реальные readers ожидают BASE64(JSON(Envelope)) и production AAD; нужен bounded re-seal adapter без смены bcrypt/password. Preparation scope не снимать, пока все declared sources не сохранены.
- Полная цель ещё открыта: полный importer и HA bootstrap/PKI, account/bot wiring, terminal period/crash settlement, реальное измерение reserve и collector/revoke SLO,48h shadow accounting после окончательной интеграции, owner CDN device и release gates. Новые APK/повторные private-canary пробы не заменяют эти задачи.
