...
Не отправлять третьей стороне подписки, UUID, XHTTP paths, credentials или конфиги. Для первичной проверки достаточно публичного IP. Положительный IP-чек доказывает только L3/TCP-доступность; работу SNI/TLS/XHTTP/CDN и реального трафика подтверждать отдельным end-to-end клиентским запросом. Ничего в BSCHEKER пока не отправлялось. Отчёт: docs/yandex-cdn-whitelist/WHITE_LIST_OPERATOR_TEST_SERVICES_2026-09-12.md.
