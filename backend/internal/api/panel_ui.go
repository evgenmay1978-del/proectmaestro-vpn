...
function bindCustomerCDN(login){
 ...
 function cdnStatus(v){if(v.enabled===false||v.publication_verdict==='DISABLED')return 'CDN отключён';if(v.enabled!==true)return 'Состояние CDN не подтверждено';var messages={PUBLISHABLE:'доступен в подписке',NO_ENTITLEMENT:'подключение ещё не подготовлено',NO_BALANCE:'для подключения нужен остаток ГБ',PRIMARY_EXPIRED:'продлите обычную подписку',PROJECTION_PENDING:'подготовка подключения',PROJECTION_STALE:'временно недоступен, данные о расходе требуют обновления',SIDECAR_UNAVAILABLE:'подключение временно недоступно',RELEASE_MISMATCH:'временно недоступен, настройка серверов не завершена'};return 'CDN включён · '+(messages[v.publication_verdict]||'готовность подключения пока не подтверждена');}
 function show(v){if(el('d_cdn_state')!==state)return;state.textContent=cdnStatus(v)+' · остаток '+(Number(v.remaining_bytes)/1000000000).toLocaleString('ru-RU',{maximumFractionDigits:3})+' ГБ'+(!v.primary_active?' · основной доступ неактивен':'');}
