...
type CommandSender interface { Send(context.Context, Target, SignedCommand) error }
type DispatcherConfig struct { Source SnapshotSource; Leases LeaseProvider; Sender CommandSender; KeyID string; PrivateKey ed25519.PrivateKey; Clock func() time.Time }
type Dispatcher struct { cfg DispatcherConfig }

func NewDispatcher(cfg DispatcherConfig) (*Dispatcher,error) {
	if cfg.Source==nil || cfg.Leases==nil || cfg.Sender==nil || strings.TrimSpace(cfg.KeyID)=="" || len(cfg.PrivateKey)!=ed25519.PrivateKeySize || cfg.Clock==nil { return nil,ErrInvalidCommand }
...
func (d *Dispatcher) DispatchBatch(ctx context.Context,triggers []Trigger) error {
	...
	for _,target:=range targets { if err:=d.dispatch(ctx,target,coalesced[target]);err!=nil{return err} }
	return nil
}
...
func (d *Dispatcher) dispatch(ctx context.Context,target Target,operationID string) error {
	...
	return d.cfg.Sender.Send(ctx,target,signed)
}
