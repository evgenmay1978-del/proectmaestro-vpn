...
if [ -n "$LOGIN" ]; then
	olc_ssh "set -eu; : '# maestro-phase=preflight'; test -d /opt/olcrtc/rooms; test -x /opt/olcrtc/olcrtc; command -v ss >/dev/null; systemctl cat 'olcrtc-srv@.service' >/dev/null"
else
	olc_ssh "set -eu; : '# maestro-phase=preflight'; test -f '/opt/olcrtc/server.yaml'; test -x /opt/olcrtc/olcrtc; command -v ss >/dev/null; systemctl cat 'olcrtc-srv.service' >/dev/null"
fi
