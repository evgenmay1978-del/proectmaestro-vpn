...
6. **FINISH TO THE END + CLEAN UP AFTER YOURSELF.** Drive a task to its real completion (e.g. canary → on-device → fleet, not "it built"). And CLEAN UP your own cruft AS YOU GO — throwaway test peers/tokens, scratch build dirs, scp'd binaries on other hosts, dead git branches (local+remote), stale test artifacts on object storage. Don't leave junk scattered across the repo + servers + storage; take inventory periodically and remove what's done. Leaving half-finished work + mess everywhere is the owner's biggest complaint.
7. **THIS OWNER:** reply in **Russian**; distribute heavy work across the available servers (don't overload one); ⛔ HARD RULE — never harm a live paying client.
