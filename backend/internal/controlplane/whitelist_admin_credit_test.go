	...
	for _, race := range []bool{false, true} {
		t.Run(fmt.Sprint("race-", race), func(t *testing.T) {
			...
			db.must(t,
				...
				rqlite.Statement{SQL: `INSERT INTO orders(order_id,payment_code,buyer_scope,buyer_key_hmac,customer_id,tariff_version_id,amount_minor,currency,duration_days,created_at_unix,expires_at_unix,payment_state,provisioning_state,decision,confirmed_at_unix,operation_id) VALUES('future-order','FUTURE','test',?,?,'future-tariff',1,'RUB',30,1,86401,'confirmed','applied','confirmed',1,'future-operation')`, Args: []any{strings.Repeat("b", 64), customer.ID}})
			insertFuture := func() {
