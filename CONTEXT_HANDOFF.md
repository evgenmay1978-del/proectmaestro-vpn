...
## CDN-ИСПАНИЯ. 20.09.2026: Испания ходит на свой origin напрямую (без круга через Германию)

Симптом: CDN-локация Испании стабильно медленнее остальных (~520–780 мс против 280–450). Причина —
эдж Yandex CDN фетчил испанский путь через единственный ingress на S4 (Германия), а уже оттуда шёл на S1;
остальные страны шли эдж → S4 → <свой сервер>. Лишний круг Германия↔Испания на каждый запрос.

Сделано:
- на S1 nginx (:28080) добавлен приём «испанского» суффиксного пути `<DE-path>/es` с rewrite в канонический
  путь S1 (дополнение, текущий трафик не менялся); бэкап `/root/maestro-flat-cdn-origin.bak-es-direct-20260920T135010Z`;
- в Yandex CDN — группа источников `es-direct-s1` (origin S1:28080, протокол **HTTP**) и **локационное
  правило** `es-direct` (pattern `^/static/main/video/segment.ts/2c988…/es`) → эта группа, weight 1.
  ⚠️ В форме правила протокол по умолчанию подставляется HTTPS — для плоского nginx:28080 обязателен HTTP.
  ⚠️ Правила CDN применяются не мгновенно (заработало примерно через час) — не считать правку неудачной сразу.

Доказательство: в логе origin S1 появились прямые попадания от эджа Yandex на путь `/es` (в т.ч. ответы 200
реальным клиентам), до правки были только от S4. Сторож CDN: Испания ~490–510 мс. Остаточный разрыв с
другими странами — плечо «эдж↔S1», а не объезд Германии.

Откат: удалить правило `es-direct` (и тестовое `zztop-test`), при желании — группу `es-direct-s1`;
на S1 вернуть nginx из бэкапа и перезагрузить.

Остаток: тестовое правило `zztop-test` (диагностика, матчит несуществующий путь) не удалось убрать через UI.
Разбор и рецепт «компьютерного окна» браузера — в памяти `maestro-memory`: `cdn-spain-direct-2026-09-20.md`,
`browser-desktop-vscreen-2026-09-20.md`.
...
Симптом: `/sub/` и `/cdn-sub/` через nginx на 8911 отдают 502, порт 18910 не слушает.
Причина: база rqlite ~1.5 ГБ → `PRAGMA foreign_key_check` через API 11 751 мс при таймауте клиента
панели 15 с (`backend/internal/rqlite/client.go`), плюс задержки raft-commit 20+ с на догоне снапшота.
Панель 44 раза не собрала runtime (`verify voter foreign keys: rqlite: request transport failed`).

Сделано (реплицированно через rqlite, без остановки voter'ов; полный отчёт —
`docs/handoff/POST-INCIDENT-2026-09-19-rqlite.md`):
- бэкап `/root/backups/maestro-rqlite-20260919T200552Z.sqlite` (sha256 `3bee72cd…adc6a`);
- cut-off 3 суток, ledger защищён: удалено 134 037 событий (+1:1 интервалы/sources/outbox) и 155 171
  идемпотентных записей; immutable-триггеры сняты и возвращены дословно;
- восстановлены 3 441 квитанция `whitelist-balance/apply-usage` (иначе `commercial_debit_pending` → 409);
- компакция `-auto-vacuum-int` (1m разово → 24h) на S2/S3.

Итог: db 386 994 176 Б на обеих нодах (было 1.51 ГБ), `foreign_key_check` 483 мс (было 11 751 мс),
`/healthz` 200, публичный `/sub/<токен>` 200, `/cdn-sub/<токен>` 403 (штатный гейт),
таймер списаний active, проводки идут (474 655 Б + 195 569 Б + 117 365 Б за три прогона).

Открыто: retention+мониторинг метеринга (прирост ~70 МБ/сутки), третий voter rqlite, паника
`/sub/<неизвестный токен>` в `subscription_snapshot.go:67` (502 вместо 404). Runbook
`docs/handoff/INCIDENT-2026-09-19-rqlite.md` в лоб не работает — см. раздел 2 пост-инцидента.
...
- **CDN-локации в клиентах**: серверная цепочка проверена живым Xray-клиентом (9/9 успешных туннелей на
  конфиге из подписки, реальный трафик в метеринге). Версии «нужен OPTIONS», «нужен полный extra Akonit» и
  «дело в alpn» проверены и опровергнуты. `n/a` в INCY — клиентское отображение пинга XHTTP-локации (ядро
  умеет XHTTP и PQ-шифрование, обвязка не знает полей extra). Остаточная рекомендация: панели отдавать
  хостнейм края, а не фиксированный IP (DNS уже указывает на другой адрес пула);
...
Этот раздел заменяет более старые заметки о незавершённой runtime-правке WDTT/olcRTC.

- Точный head: `093f4d5c5a61d1d7df3382868f04bcba9f40af58`.
- GitHub Actions на точном SHA: Android test/build `31757049389` — success; WDTT pinned transport `31757049401` — success.
- Независимое финальное ревью: READY; Critical 0, Important 0.
- Исправлены стабильное присутствие WDTT/AVG/olcRTC в мобильном интерфейсе, безопасная публикация комнаты olcRTC, rollback и восстановление после сигнала/неоднозначного ответа.
- TV не затрагивался. Серверы, OTA, DNS, клиенты, боты и платежи этой веткой не изменялись.
- До production: отдельно подтвердить SSH identity целевого узла, затем выполнить безопасный rollout и реальные проверки WDTT RX/TX и olcRTC-трафика; только после этого разрешать OTA.
...
- Старые установленные приложения должны продолжать использовать существующий
  публичный адрес `https://wapmixx.ru:8911`; аварийное переключение не должно
...
- S1 сейчас является control-plane SPOF: на нём `maestro-panel`, основной
  `customers.json`/`orders.json`, 3x-ui и nginx для `wapmixx.ru:8911`.
