## Разрешение на разумные тесты и GitHub-эмулятор — 10.09.2026

Владелец прямо разрешил проверить функции и графику на GitHub, включая геометрию всех экранов. Разрешены адресные JVM/instrumentation-тесты изменённых сценариев и настоящий Android-эмулятор на GitHub. На ноутбуке сборки, эмулятор и тесты не запускать: он слабый, USB нужен для зарядки. Не повторять провалившуюся ARM-трансляцию; использовать существующий native x86_64 libbox из того же AAR. Эмуляторный APK — отдельный артефакт, не OTA. Реальные платежи, начисления, уведомления и клиентские аккаунты тестами не изменять.

Текущие замечания владельца: дефект закрытого века, резьба должна заполнить всю центральную область, лишняя верхняя полоса. Баланс и подключение VPN владелец затем подтвердил. Публикацию160 до устранения визуальных дефектов и просмотра результата не запускать.

Перед общей OTA имя версии остаётся1.0.160, но внутренний код должен быть больше уже установленного1016000 (план1016001), чтобы владелец тоже получил исправление. Не публиковать candidate1016000 как обновление этого телефона.

## Граница выпуска160 — 10.09.2026

Исходная версия владельца157 подтверждена на физическом телефоне; первый160 уже установлен с сохранением аккаунта. Владелец одобрил объёмную резьбу вокруг медальона, умеренное тёплое освещение и новую резную шапку. Последнийэскиз07-home-final-inline-balance-preview-20260910.png; медальон03-home-carved-medallion-preview-20260910.png. После активации показыватьлогин+карандаш, нажатие открываетсмену;«Ввести логин»только доактивации. CDN/ГБ однойстрокой. Собратьисправленную160черезGitHub и посмотретьеё на физическомAndroid передобщейOTA. Не гонятьэмуляторы/новыетесты и не менятьрабочееVPNядро/серверы/ботов/балансы. Подробности, артефакт и откат — CONTEXT_HANDOFF.md.

## Мобильная реализация160 — 09.09.2026

Разрешено довести все мобильные экраны и связанные действия по принятым референсам. Использовать исходный оригинал12 и отдельный референс00 с правильными пропорциями; не добавлять новое оформление. Сборки только GitHub; исходники не считать установленным результатом. Рабочие серверы/CDN, Telegram и баланс не перестраивать. Актуальный шаг и откат — в CONTEXT_HANDOFF.md.

## Разрешение на реализацию — 09.09.2026

Последнее указание владельца: «вот теперь все экраны и функции точь в точь подгоняешь.и функционал так же ошибки не допустимы». Разрешена реализация ВСЕХ мобильных экранов и исправление связанных действий по принятым референсам. Нормальные пропорции отдельного экрана: design/mobile-approval-2026-09-09/00-home-phone-proportions.png; исходное оформление12 и реалистичный глаз11 сохраняются. Разбор конкретных дефектов/дочерних страниц: design/mobile-approval-2026-09-09/SCREEN_REVIEW.md. Прежнее ожидание одобрения дизайна ниже заменено этим разрешением.

Сборки только GitHub, новые тесты/тестовый CI без отдельного указания запрещены. Сначала необходимая сборка и показ реальных экранов приложения, затем разрешённая штатная доставка обновления с прежней подписью. Серверный CDN, боты, платежные подтверждения и реальные балансы сохраняются. Обещание отсутствия ошибок не заменяет наблюдение работающего результата. OTA159 пока остаётся остановленной.

## Мобильные экраны — согласование владельцем, 09.09.2026

Последняя обратная связь владельца: «интерфейс как то вытянут а так нормально». Стиль референсов принят, требуется исправить пропорции. Причина представления: два портретных экрана сжаты в один узкий лист. Ориентир пропорций — один экран 945×2048, как скриншот телефона владельца; отдельная главная готовится на повторный показ. Сборку/OTA не запускать до завершения согласования экранов.

Актуальное указание: сначала показать здесь качественные референсы ВСЕХ мобильных экранов на одобрение/отклонение. Стиль строго по design/mobile-4d-references/12-approved-mobile-home-2026-09-09.png: дерево, оригинальные золотые рамы, изумрудные акценты и реалистичный глаз; сохранить вход по логину и Telegram-бот. Сейчас разрешены подготовка референсов и разбор пунктов/логики приложения. До согласования не продолжать реализацию, сборку или OTA.

Дополнительный дефект владельца: CDN уже куплен, но включение отправило в бота. Разобрать цепочку account/entitlement/server availability; отсутствие серверов или ошибка загрузки не означают отсутствие оплаченного пакета. Не менять реальные балансы и не предоставлять владельцу скрытый обход проверки покупки.

Внешний вид 1.0.159 отклонён владельцем. Раздача приостановлена: оба OTA manifest/latest возвращены на 1.0.153, GitHub release159 переведён в draft; установленный159 остаётся на устройствах. Следующий исправленный APK требует той же подписи и code>1015900. Waypoint107 сохранить. Локальные сборки и новые тесты/тестовый CI запрещены. TV, работающий VPN/CDN, боты и платежи сохранить. Ниже сведения о первоначальном выпуске159 — история, а не актуальная готовность.

## Мобильный интерфейс и OTA — разрешение 09.09.2026

Выпуск завершён: production OTA1.0.159/versionCode1015900, tag tv-v1.0.159, source6956ad75b0672b279217627542a30307a70410ef. Сохранять ту же стабильную подпись и code>1015900 для следующего обновления. Waypoint107 обязателен. На current-s1 /update/ теперь обслуживается статически nginx из /var/lib/maestro/update; одновременно обновлять публичный Yandex manifest. Панель не перезапускалась. Не запускать устаревший mirror-script с удалением старых APK; он здесь не установлен. При публикации новых тегов заранее учитывать автоматические push-workflows: пути не ограничивают tag push; дополнительные сборки/тесты без разрешения запрещены.

Владелец одобрил показанный деревянно-золотой мобильный макет. Обязательно оставить видимые кнопки «Ввести логин» и Telegram-бот, сделать реалистичный глаз с исходной изумрудной фактурой, показать действительные дни VPN и остаток CDN, выбор режима/сервера и нижнюю навигацию. Разрешены реализация, подписанная сборка в GitHub и публикация штатного OTA всем клиентам. TV сохраняет свой интерфейс. Тесты/тестовый CI и локальные сборки не разрешены. Не обещать тихую установку там, где Android требует подтверждения; сохранить подпись, данные и существующий updater. Это разрешение заменяет прежние запреты на APK/release/OTA только для данного выпуска.
