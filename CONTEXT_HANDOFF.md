...
## 0Y. DESIGN: восстановление рабочего WDTT provider bridge (15.08.2026)

- Владелец утвердил архитектуру исправления WDTT; спецификация зафиксирована в
  `docs/superpowers/specs/2026-08-15-wdtt-provider-bridge-repair-design.md`.
- Доказанный дефект мобильного клиента: нативный WDTT умеет запрашивать CAPTCHA
  и ждать `CAPTCHA_RESULT`, но `WdttManager` не обрабатывает этот обмен и имеет
  слепой таймаут READY 30 секунд. Ошибка провайдерского этапа сейчас скрыта.
- Утверждённый путь: сохранить проверенную upstream-базу, добавить
  Maestro-owned patch set, типизированные стадии запуска, безопасный Android
  CAPTCHA bridge, системные плюс официальные CA roots без TLS bypass и
  candidate/active/last-known-good ротацию комнаты в Maestro-панели.
- READY разрешён только после активного нативного carrier worker, локального
  relay, WireGuard handshake/counters на S1, двусторонних байтов и публичного
  HTTPS egress. Одного успешного старта процесса недостаточно.
- На этом checkpoint изменены только документация и планирование. Код,
  серверы, клиенты, TV, SSH-аутентификация, OTA, DNS, боты и платежи не менялись.
- Следующий обязательный шаг: владелец проверяет письменную спецификацию; после
  подтверждения создаётся TDD-план и только затем начинается реализация.
