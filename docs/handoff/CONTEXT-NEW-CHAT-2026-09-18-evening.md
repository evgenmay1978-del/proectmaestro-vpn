...
### 44.5 ФИНАЛЬНЫЙ ПИНГ-ПРОГОН (400 кругов × 4 страны, каждые 3 с, 22:51–23:13 UTC)
- TLS до эджа (cdn-test.wapmixx.ru → 188.72.103.4): 400/400 ok, p50 92 мс, p90 134, max 1569.
- 🇪🇸 es **400/400** (100%), p50 152 / p90 198 мс;  🇨🇿 cz **400/400** (100%), p50 116 / p90 158 мс;
  🇳🇱 nl **400/400** (100%), p50 108 / p90 136 мс;  🇩🇪 de **399/400** (99.75%), p50 97 / p90 124 мс.
- Единственный сбой: de 502 в 23:11:32.883Z (всего 1 из 1600 запросов).
- ВЫВОД: со стороны клиентской сети (RU, 4G) CDN-цепочка по всем четырём странам стабильна; жалобы на
  отдельные n/a в приложении не воспроизводятся автотестом → смотреть на стороне приложения (кеш подписки/таймаут)
  и сопоставлять по времени с `/run/maestro-cdn-ingress/access.log`.

## 45. Проверка здоровья серверов (19.09 09:45 UTC)

| Хост | load | mem avail | диск | сервисы | ошибки |
|---|---|---|---|---|---|
| S1 | 0.43 | 670 МБ | 38% | все active, 0 рестартов | только шум x-ui («Failed to localize message: get») и sshd-kex от сканеров |
| S2 | 0.11 | 891 МБ | 46% | все active (vpn_bot restart=1 — мой рестарт) | одна сетевая ошибка Telegram у бота |
| S3 | 0.27 | 897 МБ | 29% | все active, 0 рестартов | sshd-kex от сканеров |
| S4 | 1.30 | 1131 МБ | 61% (было 65%) | все active, 0 рестартов | sshd-kex |

- rqlite: лидер есть, strong-запросы 206–216 мс ✅. Charge-джоб идёт каждые 2 мин, списания проходят
  (09:17 — debit 1 210 413 Б по wapmix) ✅. Подписки: `/sub/` 200 (DE на 8443), `/cdn-sub/` 200 (4 ноды) ✅.
  CDN с телефона: все 4 страны 200 (122–728 мс).
- **СДЕЛАНО (гигиена/безопасность):**
  1. **S1: fail2ban НЕ был установлен**, а на хосте `PermitRootLogin yes` + `PasswordAuthentication yes` и
     **339 425** неудачных попыток входа в btmp (brute force). Установлен fail2ban (jail sshd: maxretry=5,
     findtime=10m, bantime=1h), уже забанил 2 IP; в sshd добавлены `MaxAuthTries 3`, `LoginGraceTime 20`.
     На S2/S3/S4 fail2ban уже стоял (6/3/…) — там всё было в порядке.
  2. S1: обрезаны раздутые `btmp`(125 МБ)+`btmp.1`(76 МБ), `auth.log.1`(66 МБ), `syslog.1`(38 МБ) — диск 38%.
  3. S4: удалены 10 устаревших релизов коммерческого CDN-агента (`/opt/maestro-xray-cdn-commercial/releases`,
     Sep 4–12) — 750 МБ → 247 МБ, диск 65% → **61%**; оставлен `current` (`19f88042876a-…-lease180`).
     ⚠️ Предыдущий релиз тоже удалён — автоматический откат агента теперь невозможен, при необходимости
     переустановить релиз через агент.
