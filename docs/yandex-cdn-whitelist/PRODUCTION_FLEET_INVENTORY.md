...
| --- | --- | --- | --- | --- | --- |
| S4 | Existing x-ui/VPN node and private CDN canary node; Ubuntu 24.04 x64 | x-ui and Hysteria active | `18081/18082` owned by existing `maestro-xray-cdn.service` and must be preserved; `18084/18443` clear | Time synchronization, default route, disk, and failed-unit checks sane | Current console, verified service backup and restore, node certificate, reviewed firewall plan, immutable commercial artifact binding, and demonstrated rollback under five minutes are not proven |
