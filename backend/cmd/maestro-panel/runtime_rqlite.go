...
func buildRQLitePanelRuntime(
	...
	handler, err := api.WrapLegacySubscriptions(server.Handler(), config.LegacySubscriptionUpstream, publicationSource, 10*time.Second, rqliteSubscriptionTopologyFromEnvironment())
	if err != nil {
		return nil, err
	}
...
func rqliteAPIConfigFromEnvironment() api.Config {
	return api.Config{
		...
		FlatCDNNodeFile:         os.Getenv("MAESTRO_FLAT_CDN_NODE_FILE"),
		FlatCDNUUIDSecret:       os.Getenv("MAESTRO_FLAT_CDN_UUID_SECRET"),
